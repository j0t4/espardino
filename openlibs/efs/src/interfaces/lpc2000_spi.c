...
#define SELECT_CARD_FIO()   FIO0CLR = (1<<SPI_SS_PIN)
#define UNSELECT_CARD()	IOSET0 = (1<<SPI_SS_PIN)
#define UNSELECT_CARD_FIO() FIO0SET = (1<<SPI_SS_PIN)
...
#define SPI_IODIR      IODIR0
#define SPI_IODIR_FIO      FIO0DIR
#define SPI_SCK_PIN    17   /* 	       P0.17  out */
...
void if_spiInit(hwInterface *iface)
{
	...
	{
		// setup GPIO
		SPI_IODIR_FIO |= (1<<SPI_SCK_PIN)|(1<<SPI_MOSI_PIN)|(1<<SPI_SS_PIN);
		SPI_IODIR_FIO &= ~(1<<SPI_MISO_PIN);
...
#if ( HW_ENDPOINT_LPC2000_SPINUM == 0 )
	if (io_mode == FAST_IO)	SELECT_CARD_FIO();
	else					SELECT_CARD();
	
	S0SPDR = outgoing;
	while( !(S0SPSR & (1<<SPIF)) ) ;
	incoming = S0SPDR;
	if (io_mode == FAST_IO) UNSELECT_CARD_FIO();
	else					 SELECT_CARD();
#endif
